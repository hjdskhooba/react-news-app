import { fetchNewsItemById } from "actions";
import { getNewsItemById } from "selectors"
import { connect } from 'react-redux';
import { Component } from 'react';
import Layout from "containers/Layout/index.js"
import LatestNewsList from "containers/components/LatestNewsList/index.js"

class SingleNewsPage extends Component {
    componentDidMount(){
        this.props.fetchNewsItemById(this.props.match.params.id);
    };
    renderContent(item){
        return (
        <div className="news-item-page__body news-item__inner">
            <div className="news-item-page-row blocks">
                <div className="news-item__left">
                    <a className="live-updates" href={item.link}><span> </span> Live Updates</a>
                    <h1 className="title">{item.title}</h1>
                    <p className="p">{item.description} <span><a href={item.link}>{item.source}</a></span></p>
                    <p className="p p2">Updated {item.created_at}</p>
                </div>
                <div className="news-item__right">
                    <img src={item.props.image} alt={item.props['image:alt']}/>
                    <p className="img-alt">{item.props['image:alt']}</p>
                </div>
            </div>
            <div className="bottom">
                <a href={item.link} className="source-page-btn">Source Page</a>
            </div>
        </div> 
        )
    }
    render(){
        const { item } = this.props;
        return (
            <Layout>
            <div className="news-item-page">
                <div className="container">
                    {item && this.renderContent(item)}
                </div>
            </div>
            <LatestNewsList/>
            </Layout>
        );
    };
};

const mapStateToProps = state => {
    return {
        item: getNewsItemById(state, state.newsItemPage.id)
    };
};

const mapDispatchToProps = {
    fetchNewsItemById
};

export default connect(mapStateToProps, mapDispatchToProps)(SingleNewsPage);